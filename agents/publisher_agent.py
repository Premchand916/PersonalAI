...
def _build_result(
    ...
    return {
        "thread": posts,
        "twitter_thread": posts,
        "final_status": final_status,
        "error": error,
    }


def publisher_agent_node(state: PersonalAIState) -> dict[str, object]:
...
Return ONLY 5 posts separated by "---". Nothing else."""
    ...
    if not state.get("post_to_telegram", True):
        ...
        return _build_result(validated, "completed", None)
    ...
    return _build_result(validated, "post_failed", "Unexpected publisher result.")
